...
const PartnerCommissionDetail = () => {
    ...
    const { activeCommission, commissionError, fetchCommissionDetail } = usePartnerStore();

    useEffect(() => {
        fetchCommissionDetail(commissionId);
    }, [commissionId, fetchCommissionDetail]);

    if (commissionError) return <PageErrorCard message={commissionError} backLink="/partner/commissions" backText="Back to Commissions" />;
        ...
        <div className="animate-fade-in relative">
            <div className="page-header" style={{ marginBottom: '20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    <Link to="/partner/commissions" style={{ color: 'var(--text-secondary)', textDecoration: 'none' }}>
                        <ArrowLeft size={24} />
                    </Link>
            ...
            {activeCommission.payout_statement_id && (
                <div style={{ padding: '16px', background: 'var(--bg-surface-hover)', borderRadius: 'var(--radius-sm)', marginTop: '24px' }}>
                    Associated Payout Statement: <Link to={`/partner/payouts/${activeCommission.payout_statement_id}`} style={{ color: 'var(--primary)', fontWeight: 600 }}>{activeCommission.payout_statement_id}</Link>
                </div>
            )}
        </div>
    );
