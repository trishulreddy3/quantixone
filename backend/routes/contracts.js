...
// POST /contracts — Create contract for partner (approved or pending_review allowed)
router.post('/', async (req, res, next) => {
    try {
        ...
        const allowedStatuses = ['pending_review', 'approved', 'contract_sent'];
        if (!allowedStatuses.includes(partner.status)) {
            return res.status(409).json({ error: `Partner must be in pending_review or approved status to create a contract (current: ${partner.status})` });
        }

        const contract = new Contract({ partner_id, status: 'draft' });
        await contract.save();

        // Move partner to contract_sent
...
// GET /contracts — List all contracts (with optional partner_id / status filter)
...
// GET /contracts/:id/upload-url — Generate mock pre-signed URL
router.get('/:id/upload-url', async (req, res, next) => {
    try {
        const { party, filename } = req.query;
        if (!party || !['partner', 'admin'].includes(party)) {
            return res.status(400).json({ error: "party must be 'partner' or 'admin'" });
        }
        const contract = await Contract.findById(req.params.id);
        if (!contract) return res.status(404).json({ error: "Contract not found" });

        const s3Key = `contracts/${req.params.id}/${party}_${filename || 'document.pdf'}`;
        // In production, generate a real S3 pre-signed PUT URL here
...
router.post('/:id/admin-countersign', async (req, res, next) => {
    try {
        ...
        const partnerId = typeof contract.partner_id === 'object' ? contract.partner_id._id : contract.partner_id;
        await Partner.findByIdAndUpdate(partnerId, { status: 'active' });
...
// POST /contracts/:id/terminate — Terminate contract AND suspend partner
router.post('/:id/terminate', async (req, res, next) => {
    try {
        ...
        // Also suspend the partner when terminating their contract
        const partnerId = typeof contract.partner_id === 'object' ? contract.partner_id._id : contract.partner_id;
        await Partner.findByIdAndUpdate(partnerId, { status: 'suspended' });
